...
for (ct in uniq_celltypes) {
    ...
    hash_ids_ct <- submeta$Sample
    # get results
    if (cidx == 1){
      results <- cbind.data.frame(celltypes, hash_ids_ct, predicted_age)
    }
    else {
      result <- cbind.data.frame(celltypes, hash_ids_ct, predicted_age)
...
colnames(results.df) <- c('celltype','sample','predicted_age')
write.csv(results.df,"./ripr_predicted_age_singlecell.csv",row.names=FALSE)

...
for (ct in uniq_celltypes) {
    ...
    # get results
    if (cidx == 1){
      results <- cbind.data.frame(celltypes, hash_ids_ct, predicted_age, umap1, umap2)
    }
    else {
      result <- cbind.data.frame(celltypes, hash_ids_ct, predicted_age, umap1, umap2)
      #print(dim(results))
      #print(dim(result))
...
colnames(results.df) <- c('celltype','hash_id','predicted_age','umap1','umap2')
write.csv(results.df,"./results/baseline_predicted_fraction_singlecell.csv",row.names=FALSE)

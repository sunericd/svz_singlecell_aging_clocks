...
for (ct in uniq_celltypes) {
    ...
    # get results
    if (cidx == 1){
      results <- cbind.data.frame(celltypes, hash_ids_ct, batches, condition, predicted_age)
    }
    else {
      result <- cbind.data.frame(celltypes, hash_ids_ct, batches, condition, predicted_age)
...
colnames(results.df) <- c('celltype','hash_id','batch','condition','predicted_fraction')
write.csv(results.df,"./results/parabiosis_predicted_fraction_singlecell.csv",row.names=FALSE)

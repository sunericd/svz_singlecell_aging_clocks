...
for (ct in uniq_celltypes) { # look at each cell type
  ...
  # get results
  if (cidx == 1){
      results <- cbind(celltypes, hash_ids_ct, batches, condition, predicted_age)
  }
  else {
      result <- cbind(celltypes, hash_ids_ct, batches, condition, predicted_age)
...
write.csv(results.df,"./results/parabiosis_predicted_fraction.csv",row.names=FALSE)

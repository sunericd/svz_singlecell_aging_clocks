...
# Bootstrap bioage, parabiosis then exercise
...
#=====================================================================
#=====================================================================
## BootstrapCell Bioage Predictions- Parabiosis
...
df <- filter(d, Celltype == "Neuroblast")

##################
# Neuroblast Highlight
##################
ggplot(df, aes(x=Pred, fill=Sample, color=Sample)) +
 ...
 scale_color_tableau() +
 theme_classic() +
 xlim(c(0, 30)) + ylim(c(0, 0.32)) +
 ylab("Density") + xlab("Predicted Age")
ggsave("plots/parabiosis_Neuroblast_histo_bio.pdf", width=4.81, height=3.11)
...
ggplot(df, aes(x = Pred, y = Mouse, color=Sample, fill=Sample))+
        ...
        scale_color_tableau() +
        theme_classic()
ggsave("plots/parabiosis_histo_facs_mouse_bothCohorts_neuroblast_bio.pdf", width=6.76, height=3.4)
...
ggplot(filter(d, Batch == 1), aes(x=Pred, fill=Sample, color=Sample)) +
 ...
 xlim(c(0, 30)) +
 ylab("Density") + xlab("Predicted Age")
ggsave("plots/parabiosis1_histo_all_bio.pdf", width=6.74 , height=3.09)
...
ggplot(filter(d, Batch == 2), aes(x=Pred, fill=Sample, color=Sample)) +
 ...
 xlim(c(0, 30)) +
 ylab("Density") + xlab("Predicted Age")
ggsave("plots/parabiosis2_histo_all_bio.pdf", width=6.74 , height=3.09)
...
# aNSC-NPC Highlight
##################
ggplot(filter(d, Batch =="R2", Celltype == "aNSC_NPC") , aes(x=Pred, fill=Sample, color=Sample)) +
 geom_density(alpha = 0.7) +
 scale_fill_tableau() +
 scale_color_tableau() +
 theme_classic() +
 ylim(c(0, 0.25)) +
 ylab("Density") + xlab("Predicted Age")
ggsave("plots/exercise_histo_aNSC_bio.pdf", width=5, height=2.1)
...
ggplot(filter(d, Batch =="R2"), aes(x=Pred, fill=Sample, color=Sample)) +
 ...
 ylab("Density") + xlab("Predicted Age")
ggsave("plots/exercise_histo_all_bio.pdf", width=6.74 , height=3.09)

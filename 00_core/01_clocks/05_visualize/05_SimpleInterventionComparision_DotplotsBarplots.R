...
setwd("~/Dropbox/SvzClockV5/bootstrap")
d <- readRDS("../00_bootstrap/data/parabiosis_predictions.rds")
...
et <- et %>% 
  ...
  summarize(Effect = mean(Effect)) # average across two cohorts
...
d <- readRDS("../00_bootstrap/data/exercise_predictions.rds")
...
epb$Condition <- factor(epb$Condition, levels = rev(c("OldBlood", "YoungBlood",
										"OldExerciseEffect", "YoungExerciseEffect")))


ggplot(data = epb, mapping = aes(x = Celltype, y = Condition, size = abs(Effect), color = -Effect)) +
        geom_point() +
        ...
        scale_size_area(
            ...
            limits = c(0, 5),
            oob = scales::squish)
ggsave(paste0("plots/dotplot_simpleCombo_", Sys.Date(), "_Aug25.pdf"), width=2.22 , height=2.16, useDingbats=F)
...
ggplot(epb, aes(x = " ", y = -Effect, fill = Condition)) +
	...
	#geom_hline(yintercept=c(-4, -2), size = .2, linetype = "dashed")
ggsave(paste0("plots/barplot_simpleCombo_", Sys.Date(), "_Sep22.pdf"))

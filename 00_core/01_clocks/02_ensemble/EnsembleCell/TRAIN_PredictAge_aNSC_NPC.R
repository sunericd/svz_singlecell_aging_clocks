...
fileids = c("aNSC_NPC", "Astrocyte_qNSC", "Microglia", "Oligodendro", "Endothelial", "Neuroblast")
n_repeat = 20
...
# function for equal-sized k-nearest neighbors clustering (NOTE: Random clustering is better)
...
for (fileid in fileids){
    ...
    uniq_celltypes <- rep(fileid,n_repeat) # will create n_repeat number of models for this cell type
    ...
    for (ct in uniq_celltypes) { # look at each cell type
      ...
      if (normalization == TRUE){
        #clustered_data = t(scale(t(clustered_data), center=FALSE, scale=colSums(t(clustered_data))*1e-4)) # OLD normalization code
      ...
      savename <- paste("./models/", fileid, test.method, clustering.method, predictive.model, N, count_idx, sep="_")

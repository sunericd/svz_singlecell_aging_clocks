...
for (ct in uniq_celltypes) { # look at each cell type
  ...
  for (h in uniq_ids) { 
    ...
    f <- mean(submeta[submeta$hash.ID == h, ]$Prolif_Lineage_Fraction_of_SVZ)
...
# separate train test
  ...
  #if (test.method == "LeaveOne"){
  #  par(pty="s")
  #  plot(uniq_ages,mean_preds,main=ct,xlab="Actual age",ylab="Mean predicted age")
  #}
  
  #par(pty="s")
  #plot(actuals,preds,main=ct,xlab="Actual age",ylab="Mean predicted age")
...
colnames(results.df) <- c('celltype','hash_id','condition','model_num','actual_frac','predicted_frac')
filename <- paste("./results/", fileid, 'fraction', test.method, clustering.method, predictive.model, N, 'LeaveOneBatchOut', sep="_")
